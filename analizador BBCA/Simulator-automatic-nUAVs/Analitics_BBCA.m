...
ax1 = subplot(4,1,1);
b = bar(ax1,[distReal', distTotalDirect',distTotalBBCA']);
b(1).FaceColor = 'y';
b(2).FaceColor = 'r';
b(3).FaceColor = 'b';
...
ax3 = subplot(4,1,3);
b = bar(ax3,[timeTotalDirect',timeTotalBBCA']);
b(1).FaceColor = 'r';
b(2).FaceColor = 'b';
grid on
...
annotation('textbox',[.45 0.96 .3 .04],'String',['Simulation Time : ',num2str(tSim),' s'],'EdgeColor','blue')
 ...
 print(figAnalitics,[ruta 'img_statistics_',air.typeNav,'_',num2str(k)],'-dpdf');  

 %% Inserto registro en log 'outputLog' 
    outputLog = fopen([ruta 'outputLog_',air.typeNav,'_',num2str(k),'.csv'],'w');
